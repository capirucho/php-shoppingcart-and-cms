...
<?php
	...
	if ( check_input( $_POST ) ) {
		...
		//validate email here also just in case frontend validation failed
		if (!filter_var($emailAddress, FILTER_VALIDATE_EMAIL)) {
			$message = "Please enter a valid email.";
			header("Location: register.php?invalidEmail=$message");
			exit();
		}		
		...
		$insertCustomerQuery = "insert into ".$customerTable." values ('','".$firstName."', '".$lastName."', '".$emailAddress."', '".$username."', '".$password."', '".$phone."', '".$address."', '".$city."', '".$state."', '".$zipcode."', '".$ccType."', '".$ccNumber."', '".$ccExpDate."', '".$hash."', default);";


		if ($db->query($insertCustomerQuery)) {
			//get the latest Id generated by the last insert to customer table
			$customerId = $db->insert_id;

			//send verification email to user to verify user
			...
			Your account has been created, you can login with the following credentials after you activate your 
			account by clicking on the link below.

			Username: ".$username."
			Password: (The password you provided in the registration form).

			Please click the link below to activate your account:
			http://rfiguero.userworld.com/LearninPHPandMySQL/final_project/storefront/verify_user.php?verifyEmail=".$emailAddress."&hash=".$hash."
